...
// If did_free=1 all leaf mappings must already have been removed, if
// did_free=1 and unfreed leaf is found enter panic mode.
void
freewalk(pagetable_t pagetable, int did_free)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      ...
      freewalk((pagetable_t)child, did_free);
      pagetable[i] = 0;
    } else if((pte & PTE_V) && (did_free)){
      panic("freewalk: leaf");
    }
  }
